...
private:
  template <typename T>
  static constexpr bool is_named_var(std::string_view name, const T &) {
    if (name.empty()) {
      return false;
    }
  ...
  static inline void format_impl(std::ostringstream &oss, const std::string &s,
                                 size_t &pos) {
  ...
  static void format_impl(std::ostringstream &oss, const std::string &s,
                          size_t &pos, const T &first, const Args &...rest) {
...
public:
  ...
  template <typename... Args>
  static void eprint(const std::string &s, const Args &...args) {
    std::cerr << format(s, args...);
  }

  template <typename... Args>
  static void eprintln(const std::string &s, const Args &...args) {
  ...
  template <typename... Args>
  static void dbg(std::ostringstream &oss, const char *names,
                  const Args &...args) {
...
public:
  ...
  static inline bool ends_with(const std::string &s,
...
private:
  using ValueType =
      typename std::conditional_t<std::is_void_v<T>, std::monostate, T>;
...
public:
  ...
  void begin_session(const std::string &name,
                     const std::string &filepath = "profile-data.json") {
...
// In the case that you would like to profile regardless of build mode,
// uncomment this #define PROFILE
